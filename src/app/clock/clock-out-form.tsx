...
import { clockOut, type ClockOutState } from "@/app/actions";
import type { BreakWindow } from "@/lib/ninku";

const initialState: ClockOutState = { status: "idle", message: "" };
...
export function ClockOutForm({
  ...
  const action = clockOut.bind(null, entryId);
  ...
  return (
    <form action={formAction} className="flex flex-col gap-4">
      ...
      <button
        type="submit"
        disabled={isPending}
        className="w-full rounded-lg bg-orange-600 px-5 py-6 text-xl font-bold text-white shadow-sm active:bg-orange-700 disabled:opacity-50"
      >
        {isPending ? "処理中…" : "作業終了（退場）"}
      </button>
    </form>
  );
}
